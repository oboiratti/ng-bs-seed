...
export class GeneralLookupComponent implements OnInit {
  ...
  save() {
    ...
    this.settingsService.save(this.modelName, this.record).subscribe(
      res => {
        this.blockForm.stop()
        this.closeForm()
        this.fetchRecords()
    ...
    MessageDialog.confirm(
      ...
      .then(confirm => {
        if (confirm.value) {
          ...
          this.settingsService.destroy(this.modelName, id).subscribe(
            res => {
              this.blockForm.stop()
              this.closeForm()
              this.fetchRecords()
  ...
  private fetchRecords() {
    ...
    this.settingsService.fetch(this.model.name).subscribe(
      res => {
        this.blockForm.stop()
        this.records = res
      },
