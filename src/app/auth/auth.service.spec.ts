  ...
  describe('#authenticate', () => {
    ...
    it('should return response object with expected user', () => {
      authService.authenticate(params).subscribe(res => {
        expect(res).toEqual(
          expectedResponse,
          'Should return a response object with data of type User'
        )
    ...
    it('should return success: false in response object', () => {
      authService.authenticate(params).subscribe(res => {
        expect(res.success).toEqual(
          false,
          'Should return a response object with success: false'
        )
        expect(res.data).toBeNull(
          'Should return response object with data: null'
        )
      })
  ...
  describe('#setUser', () => {
    ...
    it('should set currentUser to the user passed to it', () => {
      ...
      expect(authService.currentUser).toEqual(
        expectedUser,
        'Should store user in currentUser variable'
      )
    })
    ...
    it('should store the user passed to it in local storage with the key currentUser', () => {
      ...
      expect(JSON.parse(localStorage.getItem('currentUser'))).toEqual(
        expectedUser,
        `Should store user in localstorage with key 'currentUser'`
      )
    })
  })
  ...
  describe('#removeUser', () => {
    it('should remove user from currentUser variable', () => {
      ...
      expect(authService.currentUser).toBeNull(
        'Should remove user from currentUser variable'
      )
    })
  ...
  describe('#isLoggedIn', () => {
    ...
    it('should return true when #setUser is called', () => {
      ...
      expect(authService.isLoggedIn()).toBeTruthy(
        'Should return true after #setUser is called'
      )
    })
    ...
    it('should return false when #removeUser is called', () => {
      ...
      expect(authService.isLoggedIn()).toBeFalsy(
        'Should return false after #removeUser is called'
      )
    })
  })
