...
export class RoleComponent implements OnInit, OnDestroy {
  ...
  private fetchRoles() {
    ...
    this.roles$ = this.store.pipe(select(fromRoles.getRoles))
  }

  private fetchClaims() {
    // this.claims$ = this.roleService.claims()

    this.claims$ = this.store.pipe(select(fromRoles.getClaims)).pipe(
      tap((claims) => {
        if (claims.length === 0) {
          this.store.dispatch(roleActions.loadClaims())
        }
      })
    )
  }
